...
for(var n in legs){
	var tableSection = Ti.UI.createTableViewSection({
		headerTitle: legs[n].headerTitle,
		//footerTitle: legs[n].footerTitle
	});
	for ( var i = 0, j = legs[n].rowData.length; i < j ; i++){
		var theRow = Ti.UI.createTableViewRow({
			color: "#fc6060",
			font:{
				color: "#FFF"
			},
			header: legs[n].headerTitle,
			title: legs[n].rowData[i].name,
			steps: legs[n].rowData[i].explaination,
			start: legs[n].rowData[i].image,
			finish: legs[n].rowData[i].image2,
			hasChild: true
		});
