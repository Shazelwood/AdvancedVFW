...
for(var n in abs){
	var tableSection = Ti.UI.createTableViewSection({
		headerTitle: abs[n].headerTitle,
		//footerTitle: abs[n].footerTitle
	});
	for ( var i = 0, j = abs[n].rowData.length; i < j ; i++){
		var theRow = Ti.UI.createTableViewRow({
			color: "#fc6060",
			font:{
				color: "#FFF"
			},
			header: abs[n].headerTitle,
			title: abs[n].rowData[i].name,
			steps: abs[n].rowData[i].explaination,
			start: abs[n].rowData[i].image,
			finish: abs[n].rowData[i].image2,
			hasChild: true
		});
		console.log(abs[n].rowData[i].explaination.length);
